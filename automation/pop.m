...
Begin
    ...
    for nid1: NodeIDs do
            ...
            if allocated >= BufferSpace then
                Nodes[nid1].Tokens := BufferSpace;
                allocated := allocated - BufferSapce;
            else
                Nodes[nid1].Tokens := allocated;
            endif;
    endfor;
...
ruleset n1: NodeIDs do
        ruleset n2: 1..NumberNodes-1 do
            rule "Rule --> Send request"
                (Nodes[Nodes[n1].Next].TailPointer < BufferSpace) & (Nodes[n1].Tokens > 0)
            ==>
            Begin
                SendRequest(n1, n2);
...
ruleset n1: NodeIDs do
    ruleset n2: NodeIDs do
            ...
            Begin
                ProcessFwdResponse(n1);
                ProcessResponse(n2);
                Nodes[n2].Tokens := Nodes[n2].Tokens + 1;
            endrule;
    endruleset;
endruleset;
    ...
    ruleset n2: NodeIDs do
        ruleset n3: 1..NumberNodes-1 do
                rule "Rule --> Process fwd_request + Send request"
                    (Nodes[n1].IncomingQueue[Nodes[n1].FrontPointer].msg_type = fwd_request) & (Nodes[Nodes[n1].Next].TailPointer < BufferSpace) & 
                    (Nodes[Nodes[n2].Next].TailPointer < BufferSpace) & (Nodes[n2].Tokens > 0) & 
                    (n1 != n2)
                ==>
                Begin
                    ProcessFwdRequest(n1);
...
ruleset n1: NodeIDs do
        ...
        ruleset n4: 1..NumberNodes-1 do
                    rule "Rule --> Send request + Send request"
                        (Nodes[Nodes[n1].Next].TailPointer < BufferSpace) & (Nodes[n1].Tokens > 0) & 
                        (Nodes[Nodes[n3].Next].TailPointer < BufferSpace) & (Nodes[n3].Tokens > 0) & 
                        (n1 != n3)
                    ==>
                    Begin
                        SendRequest(n1, n2);
                        Nodes[n1].Tokens := Nodes[n1].Tokens - 1;
                        SendRequest(n3, n4);
                        Nodes[n3].Tokens := Nodes[n3].Tokens - 1;
                    endrule;
            endruleset;
        endruleset;
    endruleset;
    ...
    ruleset n2: NodeIDs do
        ...
        ruleset n5: 1..NumberNodes-1 do
                        rule "Rule --> Process fwd_request + Send request + Send request"
                            (Nodes[n1].IncomingQueue[Nodes[n1].FrontPointer].msg_type = fwd_request) & (Nodes[Nodes[n1].Next].TailPointer < BufferSpace) & 
                            (Nodes[Nodes[n2].Next].TailPointer < BufferSpace) & (Nodes[n2].Tokens > 0) & 
                            (Nodes[Nodes[n4].Next].TailPointer < BufferSpace) & (Nodes[n4].Tokens > 0) & 
                            (n1 != n2) & 
                            (n1 != n4) & 
                            (n2 != n4)
                        ==>
                        Begin
                            ...
                            Nodes[n2].Tokens := Nodes[n2].Tokens - 1;
                            SendRequest(n4, n5);
                            Nodes[n4].Tokens := Nodes[n4].Tokens - 1;
                        endrule;
                endruleset;
            endruleset;
        endruleset;
...
ruleset n1: NodeIDs do
        ...
        ruleset n6: 1..NumberNodes-1 do
                            rule "Rule --> Send request + Send request + Send request"
                                (Nodes[Nodes[n1].Next].TailPointer < BufferSpace) & (Nodes[n1].Tokens > 0) & 
                                (Nodes[Nodes[n3].Next].TailPointer < BufferSpace) & (Nodes[n3].Tokens > 0) & 
                                (Nodes[Nodes[n5].Next].TailPointer < BufferSpace) & (Nodes[n5].Tokens > 0) & 
                                (n1 != n3) & 
                                (n1 != n5) & 
                                (n3 != n5)
                            ==>
                            Begin
                                ...
                                Nodes[n3].Tokens := Nodes[n3].Tokens - 1;
                                SendRequest(n5, n6);
                                Nodes[n5].Tokens := Nodes[n5].Tokens - 1;
                            endrule;
                    endruleset;
                endruleset;
            endruleset;

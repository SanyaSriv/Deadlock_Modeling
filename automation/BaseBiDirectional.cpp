/******************************
  Program "BaseBiDirectional.m" compiled by "Caching Murphi Release 5.4.9"
...
#define PROTOCOL_NAME "BaseBiDirectional"
#define BITS_IN_WORLD 272
#define HASHC
...
class mu_1_NodeIDs: public mu__byte
{
 public:
  ...
  mu_1_NodeIDs (const char *name, int os): mu__byte(6, 9, 3, name, os) {};
  mu_1_NodeIDs (void): mu__byte(6, 9, 3) {};
  mu_1_NodeIDs (int val): mu__byte(6, 9, 3, "Parameter or function result.", 0)
    { operator=(val); };
...
const char *mu_1_NodeIDs::values[] =
  { "NodeIDs_1","NodeIDs_2","NodeIDs_3","NodeIDs_4",NULL };
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (const char *name, int os): mu__byte(0, 4, 3, name, os) {};
  mu_1__type_0 (void): mu__byte(0, 4, 3) {};
  mu_1__type_0 (int val): mu__byte(0, 4, 3, "Parameter or function result.", 0)
  {
    operator=(val);
  };
...
class mu_1__type_1: public mu__byte
{
 public:
  ...
  mu_1__type_1 (const char *name, int os): mu__byte(0, 4, 3, name, os) {};
  mu_1__type_1 (void): mu__byte(0, 4, 3) {};
  mu_1__type_1 (int val): mu__byte(0, 4, 3, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
...
void mu_1_Node::set_self(const char *n, int os)
{
  ...
  if (name) mu_Next_AC.set_self_2(name, ".Next_AC", os + 65 ); else mu_Next_AC.set_self_2(NULL, NULL, 0);
}
 ...
 public:
  ...
  mu_1_Node& operator[] (int index) /* const */
  {
	...
	if (index==UNDEFVAL) 
	  ...
	  Error.Error("Funny index value %d for %s: NodeIDs is internally represented from 9 to 6.\nInternal Error in Type checking.",index, name);
	return array[0];
      }
#else
    return array[ index - 6 ];
...
if (n) array[i].set_self_ar(n,"NodeIDs_1", i * 68 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"NodeIDs_2", i * 68 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"NodeIDs_3", i * 68 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"NodeIDs_4", i * 68 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
}
...
class mu_1__type_8: public mu__byte
{
 public:
  ...
  mu_1__type_8 (const char *name, int os): mu__byte(1, 4, 3, name, os) {};
  mu_1__type_8 (void): mu__byte(1, 4, 3) {};
  mu_1__type_8 (int val): mu__byte(1, 4, 3, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
...
if (mu_next_hop_length.isundefined())
  mu_msg_to_send.mu_src_hops.undefine();
else
  mu_msg_to_send.mu_src_hops = mu_next_hop_length;
...
mu_msg_to_send.mu_src_hops = mu_Nodes[mu_src_nid].mu_IncomingQueue_CC[mu_Nodes[mu_src_nid].mu_FrontPointer].mu_src_hops;
{
...
mu_msg_to_send.mu_src_hops = mu_Nodes[mu_src_nid].mu_IncomingQueue_AC[mu_Nodes[mu_src_nid].mu_FrontPointer].mu_src_hops;
{
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n1.value((r % 4) + 6);
    r = r / 4;
    return tsprintf("Rule --> Send request 1, n2:%s, n1:%s", mu_n2.Name(), mu_n1.Name());
  }
  bool Condition(unsigned r)
  {
    ...
    mu_n1.value((r % 4) + 6);
    r = r / 4;
    return (mu_Nodes[mu_Nodes[mu_n1].mu_Next_AC].mu_TailPointer_AC) < (mu_BufferSpace);
  }
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    while (what_rule < 8 )
      {
	if ( ( TRUE  ) ) {
	      if ((mu_Nodes[mu_Nodes[mu_n1].mu_Next_AC].mu_TailPointer_AC) < (mu_BufferSpace)) {
		if ( ( TRUE  ) )
		  return;
		else
		  what_rule++;
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n1.value((r % 4) + 6);
    r = r / 4;
    return tsprintf("Rule --> Send request 0, n2:%s, n1:%s", mu_n2.Name(), mu_n1.Name());
  }
  bool Condition(unsigned r)
  {
    static mu__subrange_26 mu_n2;
    ...
    mu_n1.value((r % 4) + 6);
    r = r / 4;
    return (mu_Nodes[mu_Nodes[mu_n1].mu_Next_CC].mu_TailPointer_CC) < (mu_BufferSpace);
  }
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    while (what_rule < 16 )
      {
	if ( ( TRUE  ) ) {
	      if ((mu_Nodes[mu_Nodes[mu_n1].mu_Next_CC].mu_TailPointer_CC) < (mu_BufferSpace)) {
		if ( ( TRUE  ) )
		  return;
		else
		  what_rule++;
...
public:
  ...
  {
{
for(int mu_nid1 = 6; mu_nid1 <= 9; mu_nid1++) {
{
...
/********************
  Invariant records
 ********************/
int mu__invariant_20() // Invariant "No Global Deadlock (4-Node Cycle)"
{
bool mu__quant21; 
mu__quant21 = TRUE;
{
for(int mu_n = 6; mu_n <= 9; mu_n++) {
bool mu__boolexpr22;
bool mu__quant23; 
mu__quant23 = TRUE;
{
for(int mu_m = 6; mu_m <= 9; mu_m++) {
if ( !((mu_Nodes[mu_m].mu_TailPointer_CC) == (mu_BufferSpace)) )
...
{
for(int mu_m = 6; mu_m <= 9; mu_m++) {
if ( !((mu_Nodes[mu_m].mu_TailPointer_AC) == (mu_BufferSpace)) )
...
bool mu__condition_25() // Condition for Rule "No Global Deadlock (4-Node Cycle)"
{
...
int mu__invariant_26() // Invariant "No 3-node Deadlock Cycles"
{
bool mu__quant27; 
mu__quant27 = TRUE;
{
...
bool mu__boolexpr31;
bool mu__boolexpr32;
  if (!((mu_Nodes[mu_n].mu_TailPointer_CC) == (mu_BufferSpace))) mu__boolexpr32 = FALSE ;
  else {
  mu__boolexpr32 = ((mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_TailPointer_CC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr32)) mu__boolexpr31 = FALSE ;
  else {
  mu__boolexpr31 = ((mu_Nodes[mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_Next_CC].mu_TailPointer_CC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr31)) mu__boolexpr30 = FALSE ;
  else {
  mu__boolexpr30 = ((mu_Nodes[mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_Next_AC].mu_TailPointer_AC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr30)) mu__boolexpr29 = FALSE ;
  else {
  mu__boolexpr29 = ((mu_Nodes[mu_Nodes[mu_n].mu_Next_AC].mu_TailPointer_AC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr29)) mu__boolexpr28 = FALSE ;
  else {
  mu__boolexpr28 = ((mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_TailPointer_AC) == (mu_BufferSpace)) ; 
}
if ( !(!(mu__boolexpr28)) )
...
bool mu__condition_33() // Condition for Rule "No 3-node Deadlock Cycles"
{
...
int mu__invariant_34() // Invariant "No 2-node Deadlock Cycles"
{
bool mu__quant35; 
mu__quant35 = TRUE;
{
...
bool mu__boolexpr37;
bool mu__boolexpr38;
  if (!((mu_Nodes[mu_n].mu_TailPointer_CC) == (mu_BufferSpace))) mu__boolexpr38 = FALSE ;
  else {
  mu__boolexpr38 = ((mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_TailPointer_CC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr38)) mu__boolexpr37 = FALSE ;
  else {
  mu__boolexpr37 = ((mu_Nodes[mu_Nodes[mu_n].mu_Next_AC].mu_TailPointer_AC) == (mu_BufferSpace)) ; 
}
  if (!(mu__boolexpr37)) mu__boolexpr36 = FALSE ;
  else {
  mu__boolexpr36 = ((mu_Nodes[mu_Nodes[mu_Nodes[mu_n].mu_Next_CC].mu_Next_AC].mu_TailPointer_AC) == (mu_BufferSpace)) ; 
}
if ( !(!(mu__boolexpr36)) )
...
bool mu__condition_39() // Condition for Rule "No 2-node Deadlock Cycles"
{
...
{"No 2-node Deadlock Cycles", &mu__condition_39, NULL, },
{"No 3-node Deadlock Cycles", &mu__condition_33, NULL, },
{"No Global Deadlock (4-Node Cycle)", &mu__condition_25, NULL, },
};
...
: Presentation(Simple)
{
  ...
  if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
    ...
    mu_1_NodeIDs Perm0[4];

  /********************
   declaration of class variables
  ********************/
  in = new bool[24];
 in_mu_1_NodeIDs = new int[24];
 perm_mu_1_NodeIDs = new arr_mu_1_NodeIDs[24];
 revperm_mu_1_NodeIDs = new arr_mu_1_NodeIDs[24];
    ...
    for (i=0; i<24; i++)
      ...
      if (i_mu_1_NodeIDs >= 24) { i_mu_1_NodeIDs = 0; carry = 1; } 
      else { carry = 0; } 
    }
  }
  else
...
void PermSet::SimpleToExplicit()
{
  ...
  int start_mu_1_NodeIDs[4];
  int size_mu_1_NodeIDs[4];
  bool should_be_in_mu_1_NodeIDs[24];
...
void mu_1__type_7::ArrayLimit(PermSet& Perm)
{
  ...
  bool exists;
  bool split;
  bool goodset_mu_1_NodeIDs[4];
  bool pos_mu_1_NodeIDs[4][4];
...
void mu_1__type_7::Limit(PermSet& Perm)
{
  ...
  // only if there is more than 1 permutation in class
  if (Perm.MTO_class_mu_1_NodeIDs() && count_mu_1_NodeIDs<4)
    {
      exists = FALSE;
      split = FALSE;
  ...
  while_guard = FALSE;
  while_guard = while_guard || (Perm.MTO_class_mu_1_NodeIDs() && count_mu_1_NodeIDs<4);
  while ( while_guard )
    {
  ...
  for (i0 = 0; i0 <= 2; i0++)
  {
      ...
      while_guard = FALSE;
      while_guard = while_guard || count_mu_1_NodeIDs<4;

...
var RoutingPath: array[NodeIDs] of NodeIDs;
...
Procedure ProcesFwdRequest(src_nid:NodeIDs; incom_to_src_nid:NodeIDs; next_hop:NodeIDs);
/*
...
Procedure ProcesFwdResponse(src_nid:NodeIDs; incom_to_src_nid:NodeIDs; next_hop:NodeIDs);
/*
...
Begin
    ...
    for nid:0..NumberNodes-1 do
        if (nid != NumberNodes-1) then
            Network[nid][nid+1] := 1;
            RoutingPath[nid] := nid+1; /*nid --> nid + 1*/
        else
            Network[nid][0] := 1;
            RoutingPath[nid] := 0; /*NumberNodes-1 ---> node 0*/
        endif;
    endfor;
...
ruleset n1: 0..NumberNodes-1 do /*Node that wants to send a request to a node*/
    ruleset n2: 0..NumberNodes-1 do /*Final destination of the request*/
        rule "Send Request"
            ...
            (TailPointers[RoutingPath[n1]][n1] < BufferSpace)
                /* Use the variable here - ALSO CHECK THAT THIS NODE IS NOT A ROUTER*/
        ==>
        Begin
            SendRequest(n1, RoutingPath[n1], n2); /* Fire the procedure */
        endrule;
    endruleset;
endruleset;

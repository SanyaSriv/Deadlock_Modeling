...
const
  ...
  BufferSpace: 8;
  NumTokens: 7; /*Keeping it less than the buffer space for now.*/
...
/*Having some local tokens here that would be used per-hop.*/
var Tokens: array[NodeIDs] of array[NodeIDs] of 0..NumTokens;
...
Begin
    for nid1:0..NumberNodes-1 do
        ...
        if nid1 != nid2 then
            Tokens[nid1][nid2] := NumTokens; /* Every link (node) should start with maximum tokens*/
        endif;
        endfor;
    endfor;
...
ruleset n1: 0..NumberNodes-1 do 
    ruleset n2: 0..NumberNodes-1 do 
        rule "Send Request or set intent to send request"
           ...
           (Tokens[n2][n1] > 0) /*Make sure n2 has some tokens available.*/
        ==>
        Begin
            SendRequest(request, n1, n2); /*Fire the procedure*/
            Tokens[n2][n1] := Tokens[n2][n1] - 1; /*Cosume a token*/
        endrule;
    endruleset;
endruleset;
...
ruleset n1: 0..NumberNodes-1 do
    ruleset n2: 0..NumberNodes-1 do
        rule "Process Request -> Send Response"
            ...
            (Tokens[n2][n1] > 0) /*N2 has enough tokens to store n1's response*/
        ==>
        Begin
            ProcessRequest(n1, n2); /*Fire the procedure*/
            Tokens[n2][n1] := Tokens[n2][n1] - 1; /*We need to consume 1 token of N2*/
            Tokens[n1][n2] := Tokens[n1][n2] + 1; /*N1 should get its token back*/
        endrule;
    endruleset;
endruleset;

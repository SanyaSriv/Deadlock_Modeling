...
Processing a request will not change the global state
so we will not award a token here
The global state here means that it is not decrementing the messages in the system because it still adds
another one. This action only affects the local state and not the global one*/
ruleset n1: 0..NumberNodes-1 do
    ruleset n2: 0..NumberNodes-1 do
